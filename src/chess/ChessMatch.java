...
public class ChessMatch {
    ...
    private boolean testCheck(Color color){
        ...
        List<Piece> opponentPieces = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == opponentColor(color)).toList();

        for( Piece p : opponentPieces){
            boolean[][] mat = p.possibleMoves();
            if(mat[kingPosition.getRow()][kingPosition.getColumn()]){
                /*Se a posição que o rei está bater com algum dos possíveis
                movimentos de todas as peças ele está em check*/
                return true;
            }
        }
        return false;
